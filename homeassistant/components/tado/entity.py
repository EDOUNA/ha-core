...
class TadoDeviceEntity(Entity):
    ...
    def __init__(self, device_info: dict[str, str]) -> None:
        ...
        self.device_name = device_info.serial_no
        self.device_id = device_info.short_serial_no
        self._attr_device_info = DeviceInfo(
            configuration_url=f"https://app.tado.com/en/main/settings/rooms-and-devices/device/{self.device_name}",
            identifiers={(DOMAIN, self.device_id)},
            name=self.device_name,
            manufacturer=DEFAULT_NAME,
            sw_version=device_info.current_fw_version,
            model=device_info.device_type,
            via_device=(DOMAIN, self.device_name),
        )
