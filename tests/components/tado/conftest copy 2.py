...
async def async_init_tado_integration(
    ...
    with (
        patch("homeassistant.components.tado.tado_connector.Tado") as mock_tado,
        aioresponses() as m,
    ):
        mock_tado_instance = mock_tado.return_value
        mock_tado_instance._session = AsyncMock()
        mock_tado_instance.login = AsyncMock(return_value=None)
        ...
        m.get(
            ...
            payload=load_json_value_fixture(me_fixture),
        )
        m.get(
